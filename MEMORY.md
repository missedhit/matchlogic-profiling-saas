...
| **M1d** | Dockerfile + ECS Fargate + ALB + ACM + Cloudflare DNS. `/api/HealthCheck` reachable at `https://api.profiler.matchlogic.io` (200 OK, real cert). CloudFront for the API deferred to M4 when WAF lands. | ✅ 2026-04-29 |
...
- **2026-04-29** — **M1d done — `https://api.profiler.matchlogic.io/api/HealthCheck/` returns 200 OK, real cert.** Seven sub-steps shipped over two sessions. **(1)** Production multi-stage Dockerfile at [`backend/src/MatchLogic.Api/Dockerfile`](backend/src/MatchLogic.Api/Dockerfile) — sdk:8.0-alpine for build, aspnet:8.0-alpine for runtime, runs as the built-in non-root `app` user (added in .NET 8 base images), listens on :8080, ICU + curl installed, NuGet cache mount via BuildKit. Image 414 MB. **(2)** ECR repo `matchlogic-profiling-saas/api` via [`infra/cloudformation/ecr.yml`](infra/cloudformation/ecr.yml) — IMMUTABLE tags, ScanOnPush, AES256, lifecycle expires untagged after 14d / caps SHA-tagged at 10. GitHub Actions [`.github/workflows/backend-deploy.yml`](.github/workflows/backend-deploy.yml) auto-builds and pushes on every commit to main touching `backend/`, two tags per build (`latest` + `sha-<git-sha>`), GHA cache for repeat builds. IAM user `github-actions-backend-deploy` + scoped policy `profiler-saas-ecr-push`; static creds in GitHub Secrets `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` (TODO: migrate to OIDC pre-launch). **(3)** Seven SSM Parameter Store SecureStrings under `/profiler-saas/dev/` (`cognito/region`, `cognito/user-pool-id`, `cognito/client-id`, `mongodb/connection-string`, `mongodb/database-name`, `mongodb/progress-connection-string`, `mongodb/progress-database-name`). Free tier (Standard + AWS-managed `aws/ssm` KMS key). **(4+5)** [`infra/cloudformation/compute.yml`](infra/cloudformation/compute.yml) — VPC `10.0.0.0/16`, two public subnets across us-east-1a/b (ALB requires 2 AZs), no NAT/private subnets (Fargate gets public IPs to keep dev cheap; Atlas allowlist temporarily widened to `0.0.0.0/0` for the rotating Fargate IPs — TODO tighten pre-launch via VPC peering). ALB on 80, target group on 8080 with health check at `/api/HealthCheck/`, TLS 1.3 listener on 443 once cert wired. ECS cluster, CW log group (14d retention), task execution role with `ssm:GetParameters` scoped to the prefix + `kms:Decrypt` via the ssm service. Fargate task 0.5 vCPU / 1 GB, 1 desired, `EnableExecuteCommand: true` for debug, `HealthCheckGracePeriodSeconds: 60` to absorb .NET cold-start. ALB DNS: `profiler-saas-dev-alb-2006153364.us-east-1.elb.amazonaws.com`. **(6)** ACM cert for `*.profiler.matchlogic.io` issued via DNS validation against Cloudflare (CNAME pasted manually with the gotcha that Cloudflare's "Name" field auto-appends the zone, so the prefix has to be stripped before paste). ARN `arn:aws:acm:us-east-1:274020917421:certificate/afbfc418-43f6-4a7f-94df-462b78313dcc`. compute.yml extended with `CertificateArn` parameter + `HasCertificate` condition: when ARN is set, an HTTPS listener (port 443, `ELBSecurityPolicy-TLS13-1-2-2021-06`) is added and the existing HTTP listener switches from forward-to-target-group to a 301 redirect to HTTPS. Verified `curl -ki https://<alb>/api/HealthCheck/` → 200 OK and `curl http://<alb>/...` → 301 Location: https://. **(7)** Single CNAME at Cloudflare: `api.profiler` → ALB DNS, gray-cloud (DNS only — orange-cloud proxy would replace ACM-issued cert with Cloudflare's edge cert and break TLS termination at the ALB). Final smoke test: `curl https://api.profiler.matchlogic.io/api/HealthCheck/` → 200 OK with no -k, real cert. **Architecture deviation from original plan:** CloudFront is NOT in front of the API in M1d. Plan had `CloudFront → ALB → Fargate`; for dev we run `Cloudflare-DNS → ALB → Fargate` direct. CloudFront for the API will land in M4 alongside WAF where it earns its keep. CloudFront still happens for the static frontend in M2/M4. **Runbook:** [`docs/M1D-DEPLOY.md`](docs/M1D-DEPLOY.md) walks every click. **Cost while running:** ~$40/mo (Fargate ~$17 + ALB ~$22 + CloudWatch ~$1) on top of Atlas ~$57/mo when un-paused. Total ~$97/mo while running; pause Atlas + set DesiredCount=0 to drop to ~$22/mo (ALB still costs while it exists); delete the compute stack to drop to ~$0. Where: `infra/cloudformation/{ecr,compute}.yml`, `.github/workflows/backend-deploy.yml`, `backend/src/MatchLogic.Api/Dockerfile`, `docs/M1D-DEPLOY.md`. **Pre-launch cleanups carried forward:** `0.0.0.0/0` Atlas allowlist → VPC peering or static NAT IP; IAM static keys → OIDC federation; Cognito deletion protection ON; SES DEVELOPER mode wired; move all resources to org account; rotate Atlas DB password.

...
public class DataProfilingStatisticalAnalysisTests : BaseApiTest
{
    public DataProfilingStatisticalAnalysisTests() : base(DataProfilingEndpoints.PATH)
    {
    }
    ...
    private async Task<Result<StatisticAnalysisResponse>> GetStatisticalResponseAsync(Guid Id)
    {
        return await httpClient.GetAndDeserializeAsync<Result<StatisticAnalysisResponse>>($"{RequestURIPath}/Analytics?dataSourceId={Id}");
    }

    private void AssertSuccessResponse(Result<StatisticAnalysisResponse> response, Guid request)
    {
    ...
    private void AssertInvalidResponse(Result<StatisticAnalysisResponse> response, string identifier, string errorMessage)
    {

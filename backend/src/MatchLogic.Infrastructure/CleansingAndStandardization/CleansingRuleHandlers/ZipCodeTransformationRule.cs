...
/// <summary>
/// Rule for parsing postal codes from USA, Canada, and UK into components
/// Thread-safe with compiled regex patterns for high-performance processing
/// </summary>
public class ZipCodeTransformationRule : TransformationRule
{
    ...
    private static readonly string[] ZipCodeComponent = new[]
   {
    ...
    /// </summary>
    public ZipCodeTransformationRule(
        string sourceColumn,
...
/// <summary>
/// Holds a regular expression and corresponding country name for zip code parsing
/// </summary>
internal class CountryRegex
{
    private readonly Regex _regex;

    public string CountryName { get; }

    public CountryRegex(string countryName, string regexPattern)
    {
        CountryName = countryName;
        _regex = new Regex(regexPattern, RegexOptions.Compiled);
    }

    public string ExtractZip(string text, out bool isExtracted)
    {
        string zipExtracted = string.Empty;
        isExtracted = false;
        ...
        {
            isExtracted = true;
            zipExtracted = match.Groups[1].Value;
        }

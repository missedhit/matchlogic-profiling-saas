...
public class GetMasterRulesHandler : IRequestHandler<GetMasterRulesRequest, Result<GetMasterRulesResponse>>
{
    ...
    private readonly ILogger<GetMasterRulesHandler> _logger;

    public GetMasterRulesHandler(
        IMasterRecordRuleSetRepository ruleSetRepository,
        IGenericRepository<Domain.Project.DataSource, Guid> dataSourceRepository,
        ILogger<GetMasterRulesHandler> logger)
    {
    ...
    public async Task<Result<GetMasterRulesResponse>> Handle(
        GetMasterRulesRequest request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var response = new GetMasterRulesResponse();
            ...
            MasterRecordRuleSet? ruleSet = null;
            try
            {
            ...
            if (ruleSet?.Rules == null || !ruleSet.Rules.Any())
            {
                ...
                return Result<GetMasterRulesResponse>.Success(response);
            }
        ...
        catch (Exception ex)
        {
            ...
            return Result<GetMasterRulesResponse>.Error($"Error retrieving rules: {ex.Message}");
        }
    }

    private MasterRuleDto ConvertToDto(
            ...
            List<Domain.Project.DataSource> dataSources)
    {
        ...
        {
            var isSelected = rule.SelectedDataSourceIds?.Contains(ds.Id) ?? false;
            dataSourcesDict[ds.Name] = isSelected;
        }       

        return new MasterRuleDto
        {
            Id = rule.Id.ToString(),
